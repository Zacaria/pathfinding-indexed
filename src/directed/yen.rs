...
///
/// # Example
/// We will search the 3 shortest paths from node 0 to node 5. See
...
///     vec![],
/// ]);
/// let paths = graph.yen(0, |n| n == 5, 3);
/// assert_eq!(paths.len(), 3);
/// assert_eq!(paths[0], (vec![0, 2, 3, 5], 5));
/// assert_eq!(paths[1], (vec![0, 2, 4, 5], 7));
/// assert_eq!(paths[2], (vec![0, 1, 3, 5], 8));
///
/// let empty_graph = IndexedGraph::from_adjacency(vec![vec![(1, 3)], vec![], vec![]]);
/// let empty = empty_graph.yen(0, |n| n == 2, 2);
